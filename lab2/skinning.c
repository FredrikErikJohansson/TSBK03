...
void DeformCylinder()
{
	...
	// Modellkoordinater
	mat4 R1 = g_bones[0].rot;
	mat4 T1 = T(g_bones[0].pos.x, g_bones[0].pos.y, g_bones[0].pos.z);
	mat4 R2 = g_bones[1].rot;
	mat4 T2 = T(g_bones[1].pos.x, g_bones[1].pos.y, g_bones[1].pos.z);

	// Animated bones
	mat4 Mbone1_prime = Mult(T1, R1);
	mat4 Mbone2_prime = Mult(T2, R2);

	// Benkoordinater
	mat4 MInvbone1 = InvertMat4(T1);
	mat4 MInvbone2 = InvertMat4(T2);

	mat4 M1 = Mult(Mbone1_prime, MInvbone1);
	mat4 M2 = Mult(Mbone1_prime, Mult(Mbone2_prime, Mult(MInvbone2, MInvbone1)));
